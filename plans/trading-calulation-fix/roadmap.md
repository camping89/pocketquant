...
- **R4**: OKX `OrderResult.commission` là **accumulated snapshot** (khớp `accFillSz`/`avgPx`), paper là **per-fill**. Collector backtest cộng dồn `+= result.commission` per fill — an toàn với paper (per-fill) nhưng nếu R4+ live equity/PnL tracker tái dùng pattern cộng dồn trên chuỗi OKX order-update sẽ **double-count**. R4 phải xử OKX như snapshot (lấy delta), không cộng dồn. *(còn mở — chuyển R4.)*
