...
Promote the validated `/tmp` one-off into `scripts/recompute_backtest_slippage.py` — a reusable, idempotent CLI that re-executes one or more existing runs through the engine at a target slippage, preserving run_id + display name, and self-verifies. This is the safe mechanism for Phase 3 and any future slippage/commission recompute.

## Requirements

- **Functional:** select runs by explicit `--run-id` (repeatable) OR `--all-finished` filter; re-run each at `--slippage-bps` (default 0.5) into the same run_id; delete stale orders/trades first; restore post-run display name; verify each run.
...
for run_id in targets:
    ...
    payload = {**before.config_snapshot, "slippage_bps": target}   # inherit ALL config, override slippage
...
- `run_single(deps, payload, run_id)` in `backtest_dispatch.py` builds an isolated sandbox; `_config_from_dict` reads `slippage_bps` from payload.
- Display name is set post-run via `set_name`; `config_snapshot.name` may be the original (`test`) — capture `before.name` and restore.

## Related Code Files

- Create: `scripts/recompute_backtest_slippage.py`
...
```
uv run python scripts/recompute_backtest_slippage.py --run-id <id> [--run-id <id> ...] [--slippage-bps 0.5] [--dry-run]
uv run python scripts/recompute_backtest_slippage.py --all-finished [--exclude-slippage 0.5] [--slippage-bps 0.5] [--dry-run]
```
- `--all-finished` selects `status=finished AND config_snapshot.slippage_bps != <target>` (skips failed runs and already-correct runs).
...
1. `config_snapshot.slippage_bps == target` and `status == finished`.
...
1. Author `scripts/recompute_backtest_slippage.py` with module docstring (usage + "run on VPS container for local-Mongo speed" note, mirroring `audit_bar_quality.py`).
...
- [ ] Script runs `--dry-run` and correctly lists the 5 finished non-0.5 runs without mutating.
