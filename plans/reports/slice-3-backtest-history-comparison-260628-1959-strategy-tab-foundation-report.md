# Slice 3 — Backtest History & Comparison + UI Foundation — Brainstorm Report

## Metadata

Priority 3/5 · Vertical slice FE+BE · Depends on: none (backtest đã decoupled khỏi subscription — slice này độc lập forward branch Slice 1/2) · Unblocks: Slice 4 (ad-hoc run dùng history store + deep-link), Slice 5 (explain trade dùng backtest run detail) · Date 2026-06-29
...
- Slice 3 phải quyết: history + compare gắn vào **subscription detail** (tái-coupling theo premise) hay vào **`/backtest` page** (theo AS-IS đã decoupled) — đây là quyết định kiến trúc lớn nhất, xem §4.
...
- FE `fetchBacktestRun` (`backtest-api.ts:117-166`) tự join: fetch run doc → nếu `finished` thì fetch `/{runId}/trades` → ghép closed trades + `open_positions` thành `positions[]`. **Logic assemble đã chuyển sang FE** (bản cũ assemble ở BE worker). Run-detail của slice 3 tái dùng được hàm này.
...
- **Scope boundary**: chỉ history + compare + deep-link foundation. KHÔNG đụng ad-hoc run form logic (đã có, slice 4 mở rộng), orders detail (slice 2), explain-trade (slice 5). Forward tab/view ngoài scope nếu chọn tách (xem §4).
...
|---|---|---|---|
| **B1** | `/backtest` page (giữ AS-IS đã decoupled) | Khớp kiến trúc hiện tại (backtest standalone); không tái-couple; ít refactor `/strategies`; backtest cross-symbol/strategy tự nhiên | Lệch premise dùng chung "subscription detail 2-tab"; slice 1/2/5 (forward) vẫn ở `/strategies` → 2 nơi xem |
...
| Doc cũ trước refactor thiếu `symbol`/`interval` top-level (A2) | `from_mongo` tolerate (fallback đọc `config_snapshot`); không hard-fail; history bắt đầu data sạch từ slice 3 |
...
- `slice-4-*-adhoc-run-*-report.md` — ad-hoc run form đã tồn tại (`backtest.tsx` + `BacktestForm`); slice 4 mở rộng (params nâng cao, ghi vào history scope A2 của slice này).
- `slice-1-*-live-equity-kpi-*-report.md`, `slice-2-*-orders-*-report.md`, `slice-5-*-explain-trade-*-report.md` — forward/live hiện ở `/strategies` `DashboardColumn` (forward-only). Nếu B1, các slice này tách khỏi backtest navigation của slice 3; nếu B2 thì dùng chung shell 2-tab.
...
- Compare scope: cho phép compare 2 run **khác scope** (cross-symbol/strategy) hay chỉ trong cùng scope? (đề xuất: cùng scope ở slice 3).
...
1. B1 vs B2 — user có muốn giữ backtest decoupled (vừa refactor) hay tái-couple vào subscription theo premise 5-slice? (block toàn bộ hướng slice 3).
...
3. Các report sibling (slice 4-5) đã được re-scout theo kiến trúc mới chưa, hay vẫn dựa premise cũ?
