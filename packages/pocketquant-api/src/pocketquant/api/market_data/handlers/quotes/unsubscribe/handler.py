...
class UnsubscribeHandler(Handler[UnsubscribeCommand, dict]):
    """Handle unsubscribing from a composite symbol."""
    ...
    async def handle(self, request: UnsubscribeCommand) -> dict:
        ...
        await self._quote_app_service.provider.unsubscribe(symbol)
        cache_key = CACHE_KEY_QUOTE_LATEST.format(symbol=symbol)
        await self._cache.delete(cache_key)

        logger.info("quote_service.unsubscribed", symbol=symbol)
