...
def test_fill_roundtrip_standalone() -> None:
    f = Fill(
        fill_id="f1", order_id="o1", symbol="BTC:BIN", side=OrderSide.BUY,
        quantity=0.5, price=65000.0, commission=0.65, slippage=0.5, timestamp=NOW,
    )
...
def test_fill_roundtrip_embedded_requires_parent_id() -> None:
    f = Fill(
        fill_id="f1", order_id="o1", symbol="BTC:BIN", side=OrderSide.SELL,
        quantity=1.0, price=100.0, commission=0.1, slippage=0.0, timestamp=NOW,
    )
...
def test_fill_from_mongo_raises_without_order_id() -> None:
    with pytest.raises(ValueError, match="order_id"):
        Fill.from_mongo({
            "fill_id": "f1", "symbol": "BTC:BIN", "side": "buy",
            "quantity": 1.0, "price": 1.0, "commission": 0.0,
            "slippage": 0.0, "timestamp": NOW,
        })


def test_order_event_roundtrip_with_enum_statuses() -> None:
    ev = OrderEvent(
        timestamp=NOW, from_status=OrderStatus.SUBMITTED,
        to_status=OrderStatus.FILLED, reason="market_fill",
    )
    assert OrderEvent.from_mongo(ev.to_mongo()) == ev


def test_order_event_initial_has_none_from_status() -> None:
    ev = OrderEvent(timestamp=NOW, from_status=None, to_status=OrderStatus.SUBMITTED,
                    reason="submit")
    doc = ev.to_mongo()
...
def test_order_roundtrip_with_embedded_events_and_fills() -> None:
    fill = Fill(fill_id="f1", order_id="o1", symbol="BTC:BIN", side=OrderSide.BUY,
                quantity=0.01, price=65000.0, commission=0.65, slippage=0.5, timestamp=NOW)
    events = [
        OrderEvent(timestamp=NOW, from_status=None, to_status=OrderStatus.SUBMITTED, reason="submit"),
...
def test_trade_roundtrip_flat_entry_exit_prefix() -> None:
    t = Trade(
        ...
        pnl=19.85, commission=1.30, duration_seconds=3600.0,
    )
...
def test_trade_nullable_order_ids_round_trip() -> None:
    ...
    t = Trade(
        ...
        pnl=10.0, commission=0.21, duration_seconds=3600.0,
    )
    assert Trade.from_mongo(t.to_mongo()) == t
...
def test_open_lot_roundtrip() -> None:
    ol = OpenLot(
        symbol="BTC:BIN", direction="LONG", entry_price=65000.0, entry_time=NOW,
        quantity=0.01, sl_price=64000, tp_price=67000, entry_order_id="o1",
...
def test_metrics_roundtrip_with_duration() -> None:
    m = BacktestMetrics(
        total_return=0.15, cagr=0.12, sharpe_ratio=1.4, sortino_ratio=2.1,
        max_drawdown=-0.08, win_rate=0.55, profit_factor=1.8, total_trades=120,
        winning_trades=66, losing_trades=54, avg_win=80.5, avg_loss=-45.2,
        avg_trade_duration=timedelta(seconds=3600), total_commission=245.6,
    )
