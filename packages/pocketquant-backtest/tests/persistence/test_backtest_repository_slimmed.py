...
def _result() -> BacktestResult:
    return BacktestResult(
        ...
        open_positions=[
            OpenLot(symbol="BTC:BIN", direction="LONG", entry_price=100.0,
                    entry_time=NOW, quantity=1.0, sl_price=90, tp_price=120,
                    entry_order_id="o1", entry_commission_portion=0.1),
        ],
        started_at=NOW, completed_at=NOW,
        status="completed", parameters={"lookback": 10},
    )
...
async def test_from_mongo_tolerates_legacy_doc_with_trades_positions(database: Database) -> None:
    ...
    legacy_doc = {
        ...
        "trades": [{"order_id": "x", "symbol": "BTC:BIN", "side": "BUY",
                    "quantity": 1.0, "price": 100.0, "commission": 0.1, "pnl": 0.0,
                    "timestamp": NOW}],
        "positions": [{"symbol": "BTC:BIN", "entry_price": 100.0, "entry_time": NOW,
                       "quantity": 1.0, "sl_price": None, "tp_price": None,
                       "exit_price": 110.0, "exit_time": NOW, "pnl": 10.0,
                       "commission": 0.21, "direction": "LONG"}],
    }
