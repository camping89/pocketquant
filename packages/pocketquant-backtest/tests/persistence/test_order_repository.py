...
def _make_order(*, order_id: str, run_id: str, strategy_id: str = "s1",
    ...
    fill = Fill(fill_id=f"f-{order_id}", order_id=order_id, symbol="BTC:BIN",
                ...
                commission=0.1, slippage=0.0, timestamp=NOW)
    events = [
        OrderEvent(timestamp=NOW, from_status=None, to_status=OrderStatus.SUBMITTED, reason="submit"),
        OrderEvent(timestamp=NOW, from_status=OrderStatus.SUBMITTED, to_status=status,
    ...
    return Order(
        ...
        events=events, fills=[fill] if status == OrderStatus.FILLED else [],
    )
