...
import { INTERVAL_MS, parseIso } from './format-helpers'
...
function syncHealth(statuses: SyncStatus[]): { variant: PillVariant; label: string } {
  ...
  for (const s of statuses) {
    ...
    const lastBar = parseIso(s.last_bar_at)
    if (lastBar) {
      const age = Date.now() - lastBar.valueOf()
...
function jobHealth(jobs: JobInfo[]): { variant: PillVariant; label: string } {
  ...
  for (const j of jobs) {
    ...
    const nextRun = parseIso(j.next_run)
    if (nextRun) {
