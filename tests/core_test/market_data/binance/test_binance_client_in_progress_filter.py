...
def _patch_now(now_ms: int):
    ...
    return patch(
        "pocketquant.core.market_data.binance.binance_client.datetime",
        fake_dt,
    )
...
class TestPaginationConsistency:
    ...
    async def test_pagination_no_duplicates_and_no_in_progress(self) -> None:
        ...
        # Ascending order — Bar.datetime is Optional; fetched bars always carry one
        timestamps = [b.datetime for b in bars if b.datetime is not None]
        assert len(timestamps) == len(bars)
